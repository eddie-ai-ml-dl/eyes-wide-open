...
# -----------------------------
def auto_orient_curve(curve, trajectory, max_samples=5):
    """
    Determine curve orientation based on first crossings.
    Returns +1 if orientation correct, -1 if inverted.
    """
    crossing_directions = []
    sample_count = 0
    last_sign = None
    for anchor in trajectory:
        ...
        if last_sign is not None and current_sign != 0 and current_sign != last_sign:
            crossing_directions.append(current_sign - last_sign)
            sample_count += 1
    ...
    if not crossing_directions:
        return 1
    mean_direction = np.mean(crossing_directions)
    return 1 if mean_direction > 0 else -1

# -----------------------------
# Determine orientation
# -----------------------------
orientation = auto_orient_curve(curve, trajectory)
...
for anchor in trajectory:
    dist_signed = signed_distance_to_curve(anchor, curve) * orientation
    current_sign = np.sign(dist_signed)
...
# Output
# -----------------------------
print(f"\nTrajectory result with auto-orientation:")
